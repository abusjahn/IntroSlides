...
set_flextable_defaults(
  theme_fun = theme_zebra, font.size = 16, font.family = 'Roboto',
...
## Glimpse at data: Find the problem?'
...
```{r}
rawdata <- rawdata |> 
mutate(`Dry Content (%)`=`Moisture Content (%)`,
       `Moisture Content (%)`=100-`Dry Content (%)`)
```

```{r, echo=FALSE}
head(rawdata,3)|> flextable()
```
...
```{r}
ggplot(data = rawdata,aes(x = `Moisture Content (%)`,fill=`Region`))+
...
rawdata |> 
  ...
  pivot_wider(names_from = `Region`, values_from = pKS) 
```
...
rawdata <- mutate(rawdata, Subregion = case_when(
  `Region`=='A' & `Moisture Content (%)` > 9 ~ 'A high',
  `Region`=='A' & `Moisture Content (%)` <= 9 ~ 'A low',
  `Region`=='D' ~ 'D') |> factor())
...
ordvars$names
factvars <- FindVars('Region',casesensitive = FALSE)
factvars$bticked
```
...
t_out <- t.test(formula=`Weight Of Sample Before Drying`~`Region`, data=rawdata,
       var.equal=var.test(formula=`Weight Of Sample Before Drying`~`Region`, 
...
```{r}
plottmp <- ggplot(rawdata,aes(Subregion,`Weight Of Empty Cup`))+
...
rawdata |> group_by(Subregion) |> 
  summarize(MeanCup=mean(`Weight Of Empty Cup`) |> roundR(4)) |> 
  pivot_wider(names_from = Subregion,values_from = MeanCup) |> 
  rename_with(~paste('Mean weight empty cups\n',.x)) |> flextable()
lm1<- lm(`Weight Of Empty Cup`~Subregion, data=rawdata)
lm1
```
...
#post-hoc
(posthoc_out <- pairwise.t.test(x = rawdata$`Weight Of Empty Cup`, g = rawdata$Subregion, 
...
```{r}
ggplot(rawdata,aes(Subregion,`Weight Of Empty Cup`))+
  geom_violin(draw_quantiles = c(.25,.5,.75))+
  geom_beeswarm()+
  ...
  ggtitle('All Subregions are equal,\nbut some Subregions are more equal than others')
```
...
```{r}
lm_out <- lm(`Weight Of Sample Before Drying` ~ `Weight Of Empty Cup`, data=rawdata)
lm_out
```
...
test_ord |> select(-desc_all) |> rename_with(~str_remove(.,'Code Of ')) |> 
  flextable() |> italic(~p<=0.05,j = 1) |> bg(~p<=0.05,j = 4,bg = 'yellow') |> 

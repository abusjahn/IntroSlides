...
pacman::p_load(conflicted,tidyverse,wrappedtools, readxl,car, flextable,
               ggbeeswarm, ggsignif, ggridges, patchwork)
...
plottmp <- ggplot(rawdata,aes(Subgroup,`Weight Of Empty Cup`))+
  ...
  ggtitle('Are all subgroups equal?')
print(plottmp)
```
...
```{r}
rawdata |> group_by(Subgroup) |> 
  summarize(MeanCup=mean(`Weight Of Empty Cup`) |> roundR(4)) |> 
  pivot_wider(names_from = Subgroup,values_from = MeanCup) |> 
...
#post-hoc
(posthoc_out <- pairwise.t.test(x = rawdata$`Weight Of Empty Cup`, g = rawdata$Subgroup, 
...
plottmp <- ggplot(rawdata,aes(Subgroup,`Weight Of Empty Cup`))+
  ...
  ggtitle('All subgroups are equal,\nbut some subgroups are more equal than others')
print(plottmp)
```

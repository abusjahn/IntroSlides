...
rawdata |> 
  ...
  pivot_wider(names_from = `Subregion`, values_from = pKS) 
```
...
-   measures NOT following a Normal distribution
...
```{r, echo=TRUE}
gaussvars <- FindVars(varnames = c('Weight Of Sample','Content'), 
                      allnames = cn(rawdata))
gaussvars
ordvars <- FindVars(c('Cup'), exclude = 'Code')
ordvars$names
factvars <- FindVars('egion',casesensitive = FALSE)
factvars$bticked
```
...
t_out <- t.test(formula=`Moisture Content (%)`~`Region`, 
                data=rawdata,
                var.equal=var.test(

...
rawdata <- rawdata |> 
  mutate(
    `Weight Of Sample After Drying`=`Weigth Of Cup + Sample After Drying`-
      `Weight Of Empty Cup`,
...
t_out <- t.test(formula=`Weight Of Sample Before Drying`~`Region`, 
                data=rawdata,
                var.equal=var.test(
...
plottmp + geom_area(data = d %>% filter(x>=t_out$statistic),aes(x,y),
  ...
  annotate('label', x=t_out$statistic,y=.0,
           label=paste('area = ', 
                       roundR(pt(t_out$statistic,t_out$parameter,lower.tail = F))), 
           hjust=-0.1, vjust=-0.1)
...
anova(lm1) |> broom::tidy() |> flextable()|> 
  theme_zebra(even_body = 'aquamarine',odd_body = 'antiquewhite')
#post-hoc
(posthoc_out <- pairwise.t.test(x = rawdata$`Weight Of Empty Cup`, 
                                g = rawdata$Subregion, 
